...
export class ChurnScanner {
    ...
    public async scan(name: string, version: string): Promise<ChurnFinding|null> {
        ...
        const tgt = ChurnScanner._parseSemver(version);
        if (!tgt) {
            return null;
        }
        ...
        const previous = ChurnScanner.findPreviousVersion(reg.versions, version);
        if (!previous) {
            return null;
        }

        const prevTriple = ChurnScanner._parseSemver(previous);
        if (!prevTriple) {
            return null;
        }

        const bump = ChurnScanner._bumpType(prevTriple, tgt);
        if (!bump) {
            return null;
        }
        ...
        const diff = FingerprintDiffer.diff(prevFp, currentFp);
        const {severity, reason} = ChurnScanner._classify(bump, diff.added.length, diff.modified.length);
     ...
     * Public because the tests exercise it directly.
     */
    public static findPreviousVersion(versions: string[], target: string): string|null {
        const tgt = ChurnScanner._parseSemver(target);
        if (!tgt) {
            return null;
        }
        ...
        for (const raw of versions) {
            ...
            if (ChurnScanner._compare(t, tgt) >= 0) {
                continue;
            }
    ...
    /**
     * We only treat stable `x.y.z` releases as potential
     * predecessors. Pre-release versions (`1.0.0-rc.1`) are filtered
     * out: most npm packages release them sporadically and they
     * pollute the "previous stable" heuristic.
     */
    ...
    private static _bumpType(prev: SemverTriple, latest: SemverTriple): ChurnBumpType|null {
        if (latest[0] > prev[0]) {
            return 'major';
        }
        if (latest[1] > prev[1]) {
     ...
     * threshold them.
     *
     * Numbers are deliberately tight — once we have telemetry from a
     * few real repos we'll know whether to widen them.
     */
    private static _classify(bump: ChurnBumpType, added: number, modified: number): {
        severity: ChurnSeverity;
        reason: string;
    } {
