...
export type RunActionIO = {
    ...
    fetch?: typeof fetch;
    /** Test seam — replaces the scan run so the runner can be tested without spawning a real OSV client. */
    runScanOverride?: typeof ScanRunner.run;
};
...
export class ActionRunner {
    ...
    public static async run(io: RunActionIO): Promise<number> {
        const inputs = ActionRunner.readInputs(io.env);
        const runScanFn = io.runScanOverride ?? ScanRunner.run;
        ...
        const scanExit = await runScanFn({
            argv: scanArgv,
            cwd: io.cwd,
            stdout: (s): number => sarifChunks.push(s),
            stderr: (s): void => io.stderr(s)
        });
         ...
         */
        if (inputs.prComment) {
            const prContext = ActionRunner.readPrContext(io.env);
            if (prContext && inputs.githubToken) {
                try {
                    await ActionRunner._postPrComment(io, prContext, inputs);
                } catch (e) {
        ...
        ActionRunner._writeOutputs(io.env, {
     ...
     */
    public static readInputs(env: NodeJS.ProcessEnv): ActionInputs {
     ...
     */
    public static readPrContext(env: NodeJS.ProcessEnv): PrContext|null {
     ...
     */
    private static async _postPrComment(
        io: RunActionIO,
        pr: PrContext,
        inputs: ActionInputs
    ): Promise<void> {
        const loaded = ActionRunner._loadConfig(io, inputs.configPath);
        if (!loaded) {
            return;
        }
    ...
    /**
     * Mirror of the scan runner's config loader — we re-read because
     * the scanner exits at the end and we need a fresh `OsvClient`
     * instance keyed against the same `.nppm/cache/`. Returns `null`
     * and logs when the config is missing or malformed.
     */
    private static _loadConfig(io: RunActionIO, configPath: string|undefined): LoadedConfig|null {
     ...
     */
    private static _writeOutputs(env: NodeJS.ProcessEnv, kv: Record<string, string>): void {
