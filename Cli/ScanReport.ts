 ...
 */
export function licenseToUnified(s: LicenseSeverity): UnifiedSeverity|null {
    switch (s) {
...
function scriptToUnified(s: ScriptSeverity): UnifiedSeverity {
    return s as unknown as UnifiedSeverity;
}
function patternToUnified(s: PatternSeverity): UnifiedSeverity {
    return s as unknown as UnifiedSeverity;
}
function binaryToUnified(s: BinarySeverity): UnifiedSeverity {
    return s as unknown as UnifiedSeverity;
}
function maintainerToUnified(s: MaintainerSeverity): UnifiedSeverity {
    return s as unknown as UnifiedSeverity;
}
function unusedToUnified(s: UnusedSeverity): UnifiedSeverity {
    return s as unknown as UnifiedSeverity;
}

/**
 * Collect findings from one project's three scan outputs into the
 * unified shape. Anything below the noise floor (e.g. `info`-only
 * heuristic counts, license=permissive) is dropped — the CLI is
 * meant for triage, not exhaustive inventory.
 */
...
export function buildProjectReport(input: ProjectScanInputs): ProjectScanReport {
    const findings: ScanFinding[] = [];

    if (input.vulnsByKey) {
        // OSV batch only returns IDs (no per-vuln severity). For a CI
        // gate "any known CVE = risk" matches npm audit's default and
        // avoids per-vuln single-query latency.
    ...
    if (input.heuristics) {
        for (const h of input.heuristics) {
            if (h.scripts.maxSeverity) {
                findings.push({
                    category: 'script',
                    severity: scriptToUnified(h.scripts.maxSeverity),
                    name: h.name,
                    version: h.version,
            ...
            if (h.patterns.maxSeverity) {
                findings.push({
                    category: 'pattern',
                    severity: patternToUnified(h.patterns.maxSeverity),
                    name: h.name,
                    version: h.version,
            ...
            if (h.binaries.maxSeverity && h.binaries.totalCount > 0) {
                findings.push({
                    category: 'binary',
                    severity: binaryToUnified(h.binaries.maxSeverity),
                    name: h.name,
                    version: h.version,
            ...
            if (h.maintainer.severity) {
                findings.push({
                    category: 'maintainer',
                    severity: maintainerToUnified(h.maintainer.severity),
                    name: h.name,
                    version: h.version,
            ...
            const licenseSev = licenseToUnified(h.license.severity);
            if (licenseSev !== null) {
                findings.push({
                    category: 'license',
                    severity: licenseSev,
    ...
    if (input.unused && input.unused.supported) {
        for (const u of input.unused.unused) {
            findings.push({
                category: 'unused',
                severity: unusedToUnified(u.severity),
                name: u.name,
                message: u.reason
            });
        }
...
export function summariseReport(projects: ProjectScanReport[]): ScanReport {
    let max: UnifiedSeverity|null = null;
    let maxRank = 0;
    let withFindings = 0;
