...
- **Bug fixes in the docs / manual** — typos, broken links, outdated
  screenshots.
- **Translations** — new languages drop in as `frontend/Util/Locales/<id>.ts`
...
5. **i18n:** every new user-visible string lands in both
   `frontend/Util/Locales/en.ts` *and* `frontend/Util/Locales/de.ts`. The
...
- Backend stays English: source, comments, error messages, log lines.
  German lives in `frontend/Util/Locales/de.ts` exclusively.
