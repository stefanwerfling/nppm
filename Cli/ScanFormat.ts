...
import {buildSarif} from './ScanSarif.js';
...
/**
 * Decide whether the run breached the configured threshold. `none`
 * disables the gate entirely; anything else compares against the
 * report's worst severity.
 */
export function shouldFail(report: ScanReport, failOn: FailOnLevel): boolean {
...
/**
 * Render the aggregated report as a compact human-readable text
 * block. Designed for CI logs: one project per section, findings
 * grouped, no colour codes (CI is often grep-piped).
 */
export function formatText(report: ScanReport, failOn: FailOnLevel): string {
    const lines: string[] = [];
...
/**
 * JSON output is `JSON.stringify(report, null, 2)` — kept as a helper
 * so the call sites in `Scan.ts` and the tests stay symmetric with
 * `formatText`.
 */
 ...
 * `Cli/ScanSarif.ts` so the JSON shape stays out of the formatter.
 */
export function formatSarif(report: ScanReport): string {
    return JSON.stringify(buildSarif(report, TOOL_VERSION), null, 2) + '\n';
}
