...
import {BinaryScanner, BinarySeverity} from '../Security/BinaryScanner.js';
...
describe('BinaryScanner.scan', () => {
    it('flags Windows-, Linux- and macOS-native code as risk', () => {
        const findings = BinaryScanner.scan([
            f('bin/foo.exe'),
            f('lib/win/foo.dll'),
            f('lib/linux/foo.so'),
            f('lib/mac/foo.dylib'),
    ...
    it('flags .wasm as info', () => {
        const findings = BinaryScanner.scan([f('dist/parser.wasm')]);
    ...
    it('summariseBinaries picks worst severity + counts risk', () => {
        const out = BinaryScanner.summarise(BinaryScanner.scan([
            f('bin/a.exe'),
            f('bin/b.dll'),
            f('build/y.node'),
            f('lib/x.wasm')
    ...
    it('summariseBinaries returns null severity when empty', () => {
        const out = BinaryScanner.summarise([]);
    ...
    it('does NOT flag a shell-script bin entry', () => {
        const findings = BinaryScanner.scan([f('bin/foo.sh', 50_000)]);
        expect(findings).toEqual([]);
    });
});