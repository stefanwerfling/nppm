...
/**
 * Thrown by `parseCliArgs` when an argument doesn't conform — e.g.
 * `--fail-on=foo` or `--concurrency=abc`. The CLI shim catches these
 * and prints the message + usage to stderr.
 */
...
export function parseCliArgs(argv: readonly string[]): CliArgs {
    const out: CliArgs = {...DEFAULTS, projects: []};
